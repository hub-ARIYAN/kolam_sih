...
"""
#convert PNG to SVG:
input_image = sys.argv[1] if len(sys.argv) > 1 else "img2.jpg"  # Your PNG image
output_dir = sys.argv[2] if len(sys.argv) > 2 else "public/results"
temp_svg = "Images/temp.svg"      # Temporary SVG file
svg_path = raster_to_svg_cv(input_image, temp_svg, threshold=120)
...
with open(os.path.join(output_dir, "output_eq.html"), "w") as f:
    f.write(desmos)
# webbrowser.open(os.path.join(output_dir, "output_eq.html"), new=2)
...
def clean_equation(eq: str) -> str:
    # 1. Fix LaTeX slashes and braces
    eq = eq.replace("\\\\", "\\")
    eq = eq.replace("\\left\\{", "{").replace("\\right\\}", "}")
    
    # 2. Fix inequality symbols
    eq = eq.replace("\\le", "<=").replace(" ", "")

    # 3. Fix "+-" → "-"
    eq = eq.replace("+-", "-")

    # 4. Optional: format braces as intervals
